...
def PROPOSED(Positions, fobj, Lb, Ub, Max_iter):
    ...
    while l < Max_iter:
        r = (((np.sqrt(npop) / np.sqrt(Max_iter)) * 2) * np.random.rand(0.2, 0.5))
        c1 = 2.05 * r
        ...
        for epoch in range(1, epochnumber):
            ...
            for r in range(Max_iter):
                for i in range(N):
                    ...
                    V[i, :] = (1 - r / (Max_iter + 1)) * V[i, :] + v
                    if c1 > 1: # Update using SOA
                        nv = np.random.rand(0, 2 * 3.14)
                        Ms = Mv - (i * Mv / Max_iter)
                        S = Ms * Positions

                        Cr = 0.5 * np.random.rand()
                        L = Cr * (cgbest[i, :] - Positions[i, :])
                        ...
                        Positions[i, :] = (G * (a + b + c)) * Positions[i]
                    else: # Udate using GSO
