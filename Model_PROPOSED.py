...
def Model_PROPOSED(Train_Data, Train_Target, Test_Data, Test_Target ,sol):
    ...
    autoencoder_lstm_mlp.add(LSTM(sol[0].astype(int)), input_shape=(1, 200))
    autoencoder_lstm_mlp.add(Dense(Train_Target.shape[1]))
    autoencoder_lstm_mlp.add(
        MLPClassifier(hidden_layer_sizes=(sol[2].astype('int'), sol[3].astype('int')),
                      random_state=5,
                      verbose=True,
                      learning_rate_init=sol[4]))
